...
- source_spec: `_bmad-output/implementation-artifacts/spec-given-fsm-navigation.md`
  summary: `resolveBootstrapPath` (orchestrator/bootstrap.ts) ignores `FsmTransition.guard` — a guarded transition is selected by the shortest-path search even when its guard would not hold at run time, so a bootstrap route can be chosen that the app will not actually take. No seed transition declares a guard today, so this is latent; deciding how a preflight path search treats a runtime predicate (evaluate with what context? reject guarded transitions? defer the choice to the first step?) is a design question, not a patch.
  evidence: Blind-hunter review of the given-fsm-navigation diff flagged `FsmTransition.guard` as unhandled; the FSM type declares the optional guard (model/fsm.ts) but the resolver filters only on `from` and sorts by contractId, and preflight has no runtime context to evaluate a guard with.
...
- source_spec: none
  ...
  evidence: Split during routing of the gh-22 deferred item (2026-09-09): the evidence-naming fix is self-contained and verifiable offline, while root-causing the timeouts needs a live CDP session against Kraken Pro and a human adjudication (spec drift vs app bug); better per-step failure evidence from the naming fix will improve this investigation's starting point. Tracked upstream by https://github.com/jstavel/reactive_testing/issues/22. RESOLVED (2026-09-09): diagnosed as action-map locator drift and fixed in spec-gh-22-action-locator-drift — Assets filter became a combobox, the pager next is an unnamed chevron icon button, and Earn left the Portfolio menu for a sidebar entry; full plan now 13/13 live.

- source_spec: `_bmad-output/implementation-artifacts/spec-gh-22-action-locator-drift.md`
  summary: The smoke contracts for the History filter and pagination assert only `state-is historyMain` — nothing validates the observable effect of the actions (the BTC checkbox actually checked, the ledger actually advanced a page), so a future action that clicks a wrong-but-successful control would still pass the plan. Add probe/validator coverage for filter and pagination effects.
  evidence: Verification-gap review of the gh-22 locator-drift diff; `model/contracts.ts` filterHistoryByAsset/paginateHistoryNext declare only state self-loops and `validators/validator-map.ts` has no effect assertions for them.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-offline-corpus-reconciliation.md`
  ...
  RESOLVED (2026-09-16): `validatorsFor` now checks own properties and returns an empty validator list for inherited keys; regression coverage pins `toString`.
