...
| Detach (not close) | Run completes / fails | `closeBrowser()` disconnects from the human's browser; the browser stays open | Disconnect failure logged, never closes the user's browser |
...
**Execution:**
- [ ] `model/schemas.ts` -- ADD `cdpUrl?: string` to `OrchestratorConfig` (default `http://127.0.0.1:9222`) -- shared config shape, AD-13
- [ ] `orchestrator/browser.ts` -- ADD CDP-attach connection mode: `chromium.connectOverCDP(cdpUrl)` (wrapped in a timeout so a stalled endpoint fails fast), reuse the authenticated context (`contexts[0]`), open a NEW tab, `goto(baseUrl)`; keep the existing anonymous `chromium.launch()` path available for local/CI tests -- AD-4 "via CDP"; on detach, reset `activeSession` in a `finally` so a later run in the same process can re-attach
- [ ] `orchestrator/browser.ts` -- CHANGE `closeBrowser()` to DETACH (disconnect) rather than `browser.close()` when attached via CDP; never close the human's browser
- [ ] `orchestrator/orchestrator.ts` -- Thread the new connection mode (config.cdpUrl) through `launchBrowser`; no change to collection/corpus logic
- [ ] `model/smoke.test-plan.ts` -- Set the smoke plan's `readySelector` to the CONFIRMED live selector `[data-testid="overview-portfolio-hero-value-text"]` (replacing the wrong `#app`); verify its `modelVersion` matches the current model before a live run
- [ ] `bin/run-smoke.ts` (NEW) -- Minimal runner entry point that calls `runTestPlan(smokeTestPlan, config)` with `{ cdpUrl, baseUrl, readySelector, corpusDir, probes }` (no `headless` — it is meaningless under CDP-attach), prints the run summary, reports an explicit error when `runTestPlan` returns zero scenarios (e.g. `modelVersion` mismatch), and leaves the attached browser running
- [ ] Add/update tests: a browser-layer test that a CDP-attached session is DETACHED (not closed) and that an unreachable `cdpUrl` fails fast; keep `npm run typecheck` clean and `npm test` passing
- [ ] Live check: run `bin/run-smoke.ts` against the live logged-in browser (Chromium `:9222` + `https://pro.kraken.com/app/home`) and confirm a corpus + `run-manifest.json` land on disk -- Epic 2 goal demonstrated for the first time
...
- Given a CDP-attached run, when it finishes (success or failure), then the human's browser is left open and running — only a disconnect occurs, never a `browser.close()`, and `activeSession` is reset so a subsequent run in the same process can re-attach.
...
- **Detach, never close.** The user's browser is not owned by the run. `closeBrowser()` must disconnect only; closing would kill the human's authenticated session. This is the highest-blast-radius rule in the story. `activeSession` is reset on detach (in a `finally`) so a later run in the same process can re-attach.
